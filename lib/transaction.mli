...
(** [create_input prev_txid vout script_sig sequence] creates a new transaction input. *)
...
(** [create_transaction version inputs outputs locktime] creates a new transaction. *)
...
(** [sign_transaction tx privkey] signs a transaction using the provided private key. *)

val transaction_to_hex : transaction -> string -> string
(** [transaction_to_hex tx signatures] converts a signed transaction to a hexadecimal string. *)

val create_and_sign_transaction :
  string -> int -> string -> int64 -> int64 -> string -> string -> string
(** [create_and_sign_transaction prev_txid vout script_sig sequence value script_pubkey privkey] creates and signs a new transaction. *)
